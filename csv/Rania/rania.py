...
def stanceF():
	...
	for row in readCSV:
		...
		if row == []:
			...
			if float(row[3]) < 30.00 and tick == 0:
				sStart = row[0]
				bool = 50
				tick = 1
			elif float(row[6]) < 45.00 and tick == 1:
				sFinish = row[0]
				tick = 2
			elif float(row[3]) < 30.00 and tick == 2:
				gFinish = row[0]
				break
	gStart = sStart
	...
	for row in readCSV:
		...
		if row == []:
			...
			if float(row[6]) < 45.00 and tick == 0:
				...
				bool = 50
				tick = 1
			elif float(row[3]) < 30.00 and tick == 1:
				sFinish = row[0]
				tick = 2
...
#Results
print("\nCalculating Anthropometric data for",filename,"\n")
print("---------------------------------------------------")
print("Walking speed (m/s):",totalDist/(1000*totalTime))
print("Cadence (steps/s):",cadence)
print("Stance percentage (%):",(sFinish-sStart)/(gFinish-gStart)*100)
print("Stride time (s):",strideTime)
print("Stride length (m):",strideLength/1000)
print("---------------------------------------------------")
